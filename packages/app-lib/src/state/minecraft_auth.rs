...
/// In-flight task deduplication for online profile fetches by account UUID.
pub(super) static IN_FLIGHT_PROFILES: Mutex<
    HashMap<
        Uuid,
        tokio::sync::broadcast::Sender<Option<Arc<MinecraftProfile>>>,
...
impl Credentials {
    ...
    async fn refresh(
        ...
        self.refresh_internal(Some(exec), false).await
    }

    async fn refresh_internal(
        &mut self,
        exec: Option<impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy>,
        force: bool,
    ) -> crate::Result<()> {
        ...
        let (pair, current_date) =
            DeviceTokenPair::refresh_and_get_device_token_opt(
                oauth_token.date,
                exec,
            )
            .await?;
    ...
    async fn online_profile_with_cache_intent(
        ...
        let uuid = self.offline_profile.id;
        {
            let mut in_flight = IN_FLIGHT_PROFILES.lock().await;
            if let Some(tx) = in_flight.get(&uuid) {
                ...
                in_flight.insert(uuid, tx);
            }
        }

        let res = self
            .fetch_online_profile_internal(stale_profile, cache_intent)
            .await;

        let mut in_flight = IN_FLIGHT_PROFILES.lock().await;
        if let Some(tx) = in_flight.remove(&uuid) {
    ...
    async fn fetch_online_profile_internal(
        &self,
        stale_profile: Option<Arc<MinecraftProfile>>,
        cache_intent: OnlineProfileCacheIntent,
    ) -> Option<Arc<MinecraftProfile>> {
        let fetch_result = minecraft_profile(&self.access_token).await;
        ...
        let fetch_result = match fetch_result {
            Err(MinecraftAuthenticationError::DeserializeResponse {
                ...
                tracing::warn!(
                    "Failed to fetch online profile for UUID {} (401 Unauthorized), attempting token refresh",
                    self.offline_profile.id
                );

                let mut refreshed = self.clone();
                let no_exec: Option<&sqlx::SqlitePool> = None;
                if Box::pin(refreshed.refresh_internal(no_exec, true))
                    .await
                    .is_ok()
                {
                    minecraft_profile(&refreshed.access_token).await
                } else {
                    Err(MinecraftAuthenticationError::DeserializeResponse {
                        status_code: StatusCode::UNAUTHORIZED,
                        raw: "Token refresh failed after 401".to_string(),
                        step: MinecraftAuthStep::MinecraftProfile,
                        source: serde_json::Error::io(io::Error::other(
        ...
        match fetch_result {
            ...
            Err(
                ...
                tracing::warn!(
                    "Failed to fetch online profile for UUID {} after token refresh, backing off: {err}",
                    self.offline_profile.id
                );
    ...
    pub async fn get_default_credential(
        ...
        {
            let mut guard = IN_FLIGHT_DEFAULT_CREDENTIAL.lock().await;
            if let Some(tx) = guard.as_ref() {
                let mut rx = tx.subscribe();
                drop(guard);
                if let Ok(res) = rx.recv().await {
                    return match &*res {
                        Ok(creds_opt) => Ok(creds_opt.clone()),
                        Err(err) => Err(crate::Error {
                            raw: Arc::clone(&err.raw),
                            source: tracing_error::TracedError::from(
                                Arc::clone(&err.raw),
                            ),
                        }),
                    };
        ...
        let res = Self::get_default_credential_internal(exec).await;
        ...
        match &*shared {
            ...
            Err(err) => Err(crate::Error {
                raw: Arc::clone(&err.raw),
                source: tracing_error::TracedError::from(Arc::clone(&err.raw)),
            }),
        }
    }

    async fn get_default_credential_internal(
        exec: impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy,
    ) -> crate::Result<Option<Credentials>> {
...
impl DeviceTokenPair {
    async fn refresh_and_get_device_token_opt(
        current_date: DateTime<Utc>,
        exec: Option<impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy>,
    ) -> crate::Result<(Self, DateTime<Utc>)> {
        if let Some(exec) = exec {
            Self::refresh_and_get_device_token(current_date, exec).await
        } else {
...
{
    const RETRY_COUNT: usize = 5; // Does command 5 times
