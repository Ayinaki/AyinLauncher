...
<template>
	...
	>
		...
		>
			<TresGroup :position="[left.x, left.y, left.z]" :rotation="[0, left.ry, 0]">
				<primitive v-if="leftScene" :object="leftScene" />
			</TresGroup>

			<TresGroup :position="[right.x, right.y, right.z]" :rotation="[0, right.ry, 0]">
...
const props = withDefaults(
	defineProps<{
		...
		leftModel?: 'CLASSIC' | 'SLIM' | 'UNKNOWN'
		rightModel?: 'CLASSIC' | 'SLIM' | 'UNKNOWN'
		debug?: boolean
	}>(),
	{ debug: false, leftModel: 'CLASSIC', rightModel: 'CLASSIC' },
)
...
const { scene: rightScene } = useSkinPreviewScene({
	selectedModelSrc: rightModelSrc,
