...
export function useSkinPreviewScene({
	...
	async function loadModel(src: string) {
		...
		try {
			...
			const { scene: loadedScene, animations } = await fetchGLTF(src)
	...
	async function loadAndApplyTexture(src: string) {
		...
		try {
			const tex = await loadSkinTexture(src)
	...
	function init() {
		isTextureLoaded.value = false
		loadAndApplyTexture(textureSrc.value)
			.then((loadedTexture) => {
				texture.value = loadedTexture
				loadedTextureSrc.value = textureSrc.value
				isTextureLoaded.value = true
				return loadModel(selectedModelSrc.value)
			})
			.then(() => {
				if (capeSrc.value) {
					return loadAndApplyCapeTexture(capeSrc.value)
				}
			})
			.catch((error) => {
