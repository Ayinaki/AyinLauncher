...
pub async fn enqueue_update_for_installation<R: Runtime>(
    ...
    let mut req_builder = client.get(update.download_url.clone()).headers(headers);
    if let Some(timeout) = update.timeout {
    ...
    let temp_file_path = temp_dir.join(format!("ayin_update_{}.tmp", uuid::Uuid::new_v4()));
    ...
    tracing::info!("Downloaded update to {temp_file_path:?} in {download_duration:?}");
